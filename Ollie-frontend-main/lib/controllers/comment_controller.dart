...
import 'package:ollie/controllers/comments_repository.dart';
...
class CommentController extends GetxController {
  ...
  Future<void> loadComments(String blogId) async {
    try {
      ...
      final result = await commentsRepository.getCommentsOnBlog(blogId);
  ...
  Future<void> postComment(String blogId) async {
    ...
    try {
      final result = await commentsRepository.commentsOnBlogs({
        "comment": message,
      }, blogId);
  ...
  Future<void> toggleLike(Comment comment) async {
    try {
      ...
      final result = await commentsRepository.likeAndReplyOnComment(
        comment.id,
  ...
  Future<void> toggleReplyLike(Comment reply, String parentCommentId) async {
    try {
      ...
      final result = await commentsRepository.likeAndReplyOnComment(reply.id, {
        "like": !reply.isLiked,
      });

      if (result['success'] == true) {
        likeStatus.value = RequestStatus.success;
        final parentIndex = mainComments.indexWhere(
        ...
        if (parentIndex != -1) {
          ...
          if (replyIndex != -1) {
            ...
            mainComments[parentIndex] = parentComment.copyWith(
