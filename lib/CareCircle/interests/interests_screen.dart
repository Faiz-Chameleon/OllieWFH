  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: controller.refreshInterestsTab,
        child: SingleChildScrollView(
          controller: controller.interestsScrollController,
          primary: false,
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(
                height: 260.h,
                child: Obx(() {
                  if (controller.getYourPostAsInteresStatus.value ==
                      RequestStatus.loading) {
                    return const Center(child: CircularProgressIndicator());
                  } else if (controller.getYourPostAsInteresStatus.value ==
                      RequestStatus.error) {
                    ...
                    children: [
                      Expanded(
                        child: PageView.builder(
                          itemCount:
                              controller.postAccordingToMyInterest.length,
                          controller: PageController(viewportFraction: 0.9),
                          onPageChanged: (index) =>
                              controller.currentPage.value = index,
                          itemBuilder: (context, index) {
                            final post =
                                controller.postAccordingToMyInterest[index];
              ...
              10.verticalSpace,
              Obx(() {
                if (controller.getBlogTopicsStatus.value ==
                    RequestStatus.loading) {
              ...
              SizedBox(
                child: Obx(() {
                  if (controller.getYourSavePostStatus.value ==
                      RequestStatus.loading) {
                    ...
                    final userPost = post["userPost"] is Map
                        ? post["userPost"] as Map
                        : const {};
                    final postId = (userPost["id"] ?? post["id"] ?? "")
                        .toString();
                    final currentUserId = controller.currentUserId;
                    final postUserId =
                        (userPost["userId"] ?? post["user"]?["id"] ?? "")
                            .toString();
                    final isOwnPost =
                        currentUserId != null &&
